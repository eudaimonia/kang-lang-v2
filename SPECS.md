...
```
 source.kang (+ imports)
    ...
    │
    ▼  kang check        → Typed AST (.tast)      AST + 类型标注 + 跨文件符号解析
    │
...
main.rs
  ...
  │     └── modules.rs  → ast.rs, scope.rs, error.rs
...
| M7 | 模块系统 | lexer + parser + AST + semantic/modules + codegen (name mangling) + CLI | 800 | 导入语法正确、跨文件符号解析、分离编译+链接正确、循环导入不崩溃 | `kang build main.kang` 自动编译依赖文件并链接，`alias.item` 访问导入项，语义测试覆盖模块场景 |
| **总计** | | | **~7,400** | | |
...
- **M7 (模块系统)** 依赖 M5 (CLI/AOT 管线) 和 M2 (语义分析)，在 M5 之后实施
...
| semantic → codegen | `ast::Program` (+ imported modules) | `TypedProgram` | `Vec<SemanticError>` |
| codegen → 外部 | `TypedProgram` | `CodeGenResult` | `CodeGenError` |
| modules (M7) | `ast::Program` × N (compilation units) | 编译调度 + 链接 | `KangError` |
...
**新增关键字**: `import`、`from` (实现在 `lexer.rs`)

**新增 AST 节点** (实现在 `ast.rs`):

```rust
// 导入语句 (仅限顶层)
pub struct ImportStmt {
    pub alias: String,         // 命名空间前缀
    pub items: Vec<String>,    // 被导入的顶层项名称列表
    pub path: String,          // 源文件路径 (相对路径字符串)
}
```
...
- `IDENT` (alias) 成为导入项的命名空间前缀
- `{ }` 内为显式导入列表，仅导入指定的顶层函数名/结构体名
- `STR_LIT` 为被导入文件的路径，相对于当前文件所在目录解析

**模块解析** (新增 `semantic/modules.rs`):

- `resolve_imports(program, file_path) -> Vec<ImportResolution>`: 解析 `import` 语句，加载被导入模块的 AST
- `compile_units(entry_file) -> Vec<CompilationUnit>`: 从入口文件出发，拓扑排序所有编译单元
- 每个编译单元先注册其顶层签名（函数签名 + 结构体定义），再在各单元体中解析符号引用
- 循环导入：签名先行注册，体在链接期解析
...
CompilationUnit {
    source_file: PathBuf,
    ast: Program,
    exports: HashMap<String, SymbolRef>,  // 顶层定义的符号
    imports: Vec<ImportStmt>,              // 导入语句
    object_file: Option<PathBuf>,          // 编译产物 .o
}
```

**符号表扩展** (实现在 `semantic/scope.rs`):

- 新增命名空间层级：`scope.resolve("alias.item")` 在两个命名空间中查找
- 模块名 (alias) 与变量名/函数名共享全局命名空间
- 每个模块的顶层符号在导入时注册到 `<alias>::<item>` 下

**语义检查** (实现在 `semantic/checker.rs`):

| # | 规则 | 错误触发条件 |
|---|------|------------|
| IM1 | 导入路径存在 | 指定的文件路径无法解析到实际文件 |
| IM2 | 导入项存在 | 指定导入的项在被导入模块中不存在 |
| IM3 | 模块名唯一 | 同一文件中不同 import 使用相同 alias |
| IM4 | 模块名不与本地符号冲突 | alias 与当前文件的函数名/变量名共享命名空间 |
| IM5 | 被导入项类型匹配 | 使用 `alias.item` 时的类型与导入源一致 |

**代码生成** (实现在 `codegen.rs` + `codegen/stmt.rs`):

- 每个编译单元独立生成 .o 文件
- LLVM 函数名使用 name mangling 避免跨模块冲突: `<filename_hash>::<func_name>`
- 导入项调用: `alias.item()` → 生成对 `<imported_file_hash>::<item>()` 的 call 指令
- 结构体类型通过 LLVM named struct type 跨模块一致

**CLI 扩展** (实现在 `main.rs`):

- `kang build entry.kang`: 递归解析 import，先编译依赖文件，再编译入口，最后链接
- `kang build` 接受多个位置参数（入口文件 + 显式依赖文件）
- 依赖文件的 .o 产物放在临时目录（`<source_file_stem>.o`）

**链接阶段**:

```
所有编译单元的 .o 文件 + libkangrt.a → 链接器 (cc/lld) → 可执行文件
```

- 链接顺序: libkangrt.a 在最后
- 重复符号检测委托给链接器
- 若用户未指定 `-o`，默认输出文件名为入口文件去掉 `.kang` 后缀
