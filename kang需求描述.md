...
- 模块导入（仅限顶层）：`import alias { item1, item2 } from "path/file.kang";`
  - 将其他文件中的顶层函数/结构体导入当前模块
  - 导入项通过 `alias.item` 访问
  - 路径相对于当前源文件所在目录解析
  - 所有顶层项默认可导入，无需 `pub`/`priv` 修饰符
  - 被导入模块独立编译为目标文件，链接期合并
...
**作用域**
- 词法作用域
- 模块命名空间：导入项通过 `alias.item` 访问，`alias` 为导入时指定的命名空间前缀

### 模块系统

**设计目标**：支持跨文件代码组织和复用，为 Kang 语言自举（用 Kang 写 Kang 编译器）提供基础。

**导入语法**：
```
import alias { item1, item2 } from "path/file.kang";
```
- `alias`：命名空间前缀，导入后通过 `alias.item` 访问
- `{ item1, item2 }`：显式导入列表，列出需要的顶层函数名或结构体名
- `"path/file.kang"`：被导入文件的路径，相对于当前源文件所在目录解析

**新增关键字**：`import`、`from`

**编译模型**：
- 每个源文件为独立编译单元（compilation unit），各自经过 lex → parse → semantic → codegen 生成 .o 文件
- 导入模块时，先编译被导入文件生成 .o，再编译导入方
- 链接阶段将所有 .o + libkangrt.a 合并为可执行文件

**循环导入**：通过分离编译支持 — 被导入模块的签名先注册，体在链接期解析。

**可见性**：所有顶层项（函数、结构体）默认可导入，v1 不设 `pub`/`priv` 修饰符。

**命名空间**：
- 导入项通过 `alias.item` 访问，不同模块的同名项通过不同 alias 区分
- 当前文件的顶层项直接通过原名访问，无需前缀
- 模块名（alias）与变量名/函数名共享命名空间
...
- 词法分析：logos
- 错误报告：ariadne
- 架构：词法→语法→语义→代码生成 四阶段，REPL 和 AOT 复用前三阶段；模块系统通过独立编译单元 + 链接实现
