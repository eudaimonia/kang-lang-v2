...
| 函数 | 说明 |
|------|------|
| `puts(s: str)` | 输出字符串到 stdout |
| `print(s: str)` | 同 puts + 换行 |
| `eprint(s: str)` | 输出到 stderr + 换行 |
...
7. **i32/f64 不可混用**: `1 + 2.0` 是编译错误
8. **哨兵错误可能漏检**: 空文件、false 布尔值、-1 整数与错误值重叠
9. **函数不支持重载**: 每个函数名在作用域内唯一 (内置函数除外)
